...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Snake = function () {
	    ...
	    (0, _createClass3.default)(Snake, [{
	        ...
	        value: function changeDirection(direction) {
	            if (this.dirtyDirection) return;
	            if (Math.abs(this.direction - direction) == 2 || this.direction == direction) return;
	...
	var Game = function () {
	    ...
	    (0, _createClass3.default)(Game, [{
	        ...
	        value: function _calculatePosition(item) {
	        ...
	        value: function _initConfig(options) {
	            this.config = (0, _assign2.default)({}, this.initConfig, options);
	            var scale = parseInt(this.config.graphSize / this.config.rectSize);
	            var fixedGraph = scale * this.config.rectSize;
	            this.config.graphSize = fixedGraph;
	            this.config.scale = scale;
	        }
	    }, {
	        key: '_initGraph',
	        value: function _initGraph() {
	            var appEl = document.getElementById('app');
	            if (this.graph) appEl.removeChild(this.graph);
	        ...
	        value: function _initEvent() {
	            ...
	            this.graph.addEventListener('keydown', function (e) {
	        ...
	        key: '_initFood',
	        value: function _initFood() {
	            for (var i = 0; i < this.config.foodCount; i++) {
	                this._produceFood();
	            }
	        }
	    }, {
	        ...
	        value: function _drawSnake() {
	            ...
	            body.map(function (item) {
	                var position = _this2._calculatePosition(item);
	                _this2.graph.childNodes[position].classList.add('snackBody');
	            });
	        }
	    }, {
	        key: '_updateSnake',
	        value: function _updateSnake() {
	            ...
	            var tail = this.snake.body[0];
	            var tailPosition = this._calculatePosition(tail);
	            this.snake.move();

	            var head = this.snake.body[this.snake.length - 1];
	            var headPosition = this._calculatePosition(head);
	            ...
	            if (this.graph.childNodes[headPosition].classList.contains('food')) {
	                ...
	                var newTail = this.snake.grow();
	                this.graph.childNodes[this._calculatePosition(newTail)].classList.add('snackBody');
	                this._produceFood();
	            }

	            this.timeFlag = setTimeout(function () {
	                return _this3._updateSnake();
	            }, 220 - this.config.speed * 20);
	        }
	    }, {
	        key: '_produceFood',
	        value: function _produceFood() {
	            var newPosition = [Math.floor(Math.random() * this.config.scale), Math.floor(Math.random() * this.config.scale)];
	            var target = this.graph.childNodes[this._calculatePosition(newPosition)];
	            if (target.classList.contains('snakeBody') || target.classList.contains('food')) {
	                this.produceFood();
	                return;
	            }
	            this.foodPosition = newPosition;
	            var position = this.foodPosition[0] * this.config.scale + this.foodPosition[1];
	            var food = this.graph.childNodes[position];
	            food.classList.add('food');
	        }
	    }, {
	        key: 'adjustSpeed',
	        value: function adjustSpeed(operation) {
	            this.graph.focus();
	            if (operation > 0) {
	                if (this.config.speed == 9) return;
	            } else {
	                if (this.config.speed == 1) return;
	            }
